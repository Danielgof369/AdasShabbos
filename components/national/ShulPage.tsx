...
export default async function ShulPage({ slug }: { slug: string }) {
  if (await currentShul()) redirect(`${rootBaseUrl()}/${slug}`);
  ...
  const joinHref = shul.hasSite ? `${shulBaseUrl(shul)}/signup` : `/join?s=${encodeURIComponent(shul.slug)}`;
      ...
      <section className="bg-navy text-cream relative overflow-hidden">
        ...
        <div className="mx-auto max-w-4xl px-4 py-14 relative">
          <p className="text-gold-soft font-display tracking-widest uppercase text-sm mb-3">
            <Link href="/" className="hover:underline">{PLATFORM.name}</Link> &middot; {campaign.seasonLabel}
          </p>
          {shul.logoDark && (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={shul.logoDark} alt={shul.name} className="h-20 sm:h-24 w-auto max-w-[320px] object-contain mb-5" />
          )}
